...
import cn.jeesoft.core.utils.ImageIOUtils;
/**
...
public class ImageUtils {
	 ...
	 */
	public static boolean ratioZoom(File srcFile, File targetFile, int newWidth,int newHeight) throws IOException {
		double Ratio = 0.0;
		if (!srcFile.isFile())
...
//		bfImage.getGraphics().drawImage(Bi.getScaledInstance(newWidth, newHeight,Image.SCALE_SMOOTH),0,0,null);
		...
		try {
			String suffix = getExtensionName(targetFile.getAbsolutePath());
			
			os = new FileOutputStream(targetFile);
			ImageIO.write((BufferedImage) Itemp, suffix, os);
		} catch (Exception ex) {
			System.out.println("图片缩放存储异常，e = "+ex);
			ex.printStackTrace();
...
//					+ ex.getMessage());
	...
	public static void reduceImg(String imgsrc, String imgdist, int widthdist,      
		...
		try {
			...
			ImageIOUtils.saveImage(tag, new File(imgdist));
		...
		try {
			...
			bfImage.getGraphics().drawImage(
					image.getScaledInstance(newWidth, newHeight,
							Image.SCALE_SMOOTH), 0, 0, null);
			ImageIOUtils.saveImage(bfImage, dist);
	...
	public static void main(String[] args) {
		try {
			if (ImageUtils.ratioZoom(new File("D:/test.JPG"), new File("D:/test1.JPG"), 300,300)) {
		...
		ratioZoom2(new File("D:/test.JPG"), new File("D:/test2.JPG"),300d);
		reduceImg("D:/test.JPG", "D:/test3.JPG", 300, 225);
	}
